...
def training_loop(
    ...
    *,
    ...
    # )  # noise + low-res input
    if task == 'diffusion':
        img_in_channels = len(dataset.variables)
        ...
        network_kwargs.img_channels = img_in_channels
    else:
    ...
    net = construct_class_by_name(**network_kwargs)  # subclass of torch.nn.Module
        ...
        for round_idx in range(num_accumulation_rounds):
            with ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                if task == 'diffusion':
                    ...
                    with torch.autocast("cuda", dtype=amp_dtype, enabled=enable_amp):
                        loss = loss_fn(outvar.to(device).to(torch.float32).contiguous(), net(invar.to(device).to(torch.float32).contiguous())
                        )
                ...
                with torch.no_grad():
                    for _ in range(num_validation_evals):
                        ...
                        if task == 'diffusion':
                            ...
                            loss_valid = loss_fn(
                                net=ddp,
                                images=img_clean_valid
                            )

                        else:
                            invar, outvar = next(validation_dataset_iterator)
                            with torch.autocast("cuda", dtype=amp_dtype, enabled=enable_amp):
                                loss_valid = loss_fn(outvar.to(device).to(torch.float32).contiguous(), net(invar.to(device).to(torch.float32).contiguous())
                                )
